...
The proposed phases follow the priorities identified through our discussions and workshops with [Client Name].

For the recommended rebuild option, the majority of the investment sits within the first phase because this is where the core platform, integrations and essential customer journeys need to be established.

Under the current scope, approximately 80% of the overall estimate sits within Phase 1.

This is important when considering phasing commercially. The proposed approach allows [Client Name] to sequence delivery, launch the core service first and introduce additional functionality afterwards, but it does not currently create a significantly lower-cost first release.
...
For the rebuild this includes the core commerce application, frontend experience, customer and account structure, product catalogue, pricing, ordering, administration functionality and the key system integrations.
...
## If [Client Name] wants a smaller initial investment

If reducing the initial commitment is a priority, we can review the Phase 1 scope with [Client Name] and identify whether a genuinely smaller first release is practical.

This would require more than simply moving secondary features into a later phase. We would need to agree which customer groups, account types or workflows could reasonably remain on an alternative process while the first version of the new platform is launched.
...
Any such change would need to consider how affected customers continue to operate in the meantime, whether temporary processes are required and whether deferring functionality creates additional development or migration work later.
...
The overall current budget estimate is:

| Approach | Amount |
|----------|--------|
| Recommended rebuild | £100,000 + VAT |

### Recommended rebuild

| Phase | Scope | Indicative hours | Indicative cost |
|-------|-------|------------------|-----------------|
| MVP / Phase 1 | Retain core current-platform functionality and deliver all Priority 1 requirements. Includes new design and custom frontend, platform foundation, CMS, customer and organisation model, customer group management, roles and permissions, customer-specific catalogues and pricing, core [Core System] syncs, online ordering, stock, search, product catalogue, checkout, delivery rules, resources, account portals, order tracking, payments, migration, analytics, accessibility and deployment. | 850 hrs | £79,500 + VAT |
| Phase 2 | Priority 2 requirements including enhanced order amendments, reorder/saved baskets/quick ordering, invoices, partial deliveries, enhanced stock functionality, auto-generated documents, and additional account information. | 75 hrs | £7,000 + VAT |
| Optional / Phase 3 | Priority 3 requirements including extended delivery scheduling, customer-specific messaging, and portal enhancements. | 25 hrs | £2,333 + VAT |
| Excluded / separately quoted | Priority 4 requirements such as advanced customer features and integrations. These should be technically scoped before pricing. | TBC | TBC |
| **Quoted project total** | **Recommended rebuild estimate excluding separately quoted Priority 4 items.** | **950 hrs** | **£100,000 + VAT** |

The objective of the phasing is to give [Client Name] a controlled route through delivery and launch while keeping the commercial implications clear.

## Current platform functionality to retain

| Requirement area | Current functionality to retain | Priority | Phase | Recommended solution | Alternative approach | [Core System] dependency |
| --- | --- | ---: | --- | --- | --- | --- |
| Administration | CMS administration and user-role management | 1 | MVP | Platform admin plus CMS/editor tooling | SaaS platform admin with custom configuration | No / Partial for account roles |
